...
select nome_cliente,count(nome_cliente) 
from deposito
group by nome_cliente;
...
select nome_cliente, sum(saldo_deposito), count(nome_cliente) 
from deposito
group by nome_cliente
order by count(nome_cliente) asc, sum(saldo_deposito) desc;

--Encontre o número de depositantes em cada agência
select nome_agencia, nome_cliente ,count(distinct nome_cliente)
from deposito
